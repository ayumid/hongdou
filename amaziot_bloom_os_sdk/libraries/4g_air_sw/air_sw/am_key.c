...
static void _gpiotimer_callback0(UINT32 tmrId)
{
    ...
    else
    {
        if(lastKeyStatus0 == 0)
        {
            ...
            uprintf("diff0:%d,elink_time0:%d",diff0,elink_time0);
            if(diff0 > 20)
            {
                //20201022
                uprintf("change gpio0 status");

//                OSAFlagSet(GpioFlgRef, 0x01, OSA_FLAG_OR);
                app_led_msgq_msg gpioMsg = {0};

                gpioMsg.msgId = LED_TASK_MSG_KEY1_MSG;
...
static void _gpiotimer_callback1(UINT32 tmrId)
{
    ...
    else
    {
        if(lastKeyStatus1 == 0)
        {
            ...
            uprintf("diff1:%d,elink_time1:%d",diff1,elink_time1);
            if(diff1 > 20)
            {//20201022
                uprintf("change gpio1 status");
                
//                OSAFlagSet(GpioFlgRef, 0x01, OSA_FLAG_OR);
                app_led_msgq_msg gpioMsg = {0};

                gpioMsg.msgId = LED_TASK_MSG_KEY2_MSG;
                send_task_send_msgq(&gpioMsg);
            }
        }
        lastKeyStatus1 = 1;    //20201016    
...
static void _gpiotimer_callback2(UINT32 tmrId)
{
    ...
    else
    {
        if(lastKeyStatus2 == 0)
        {
            ...
            uprintf("diff2:%d,elink_time2:%d",diff2,elink_time2);
            if(diff2 > 20)
            {
                //20201022
                uprintf("change gpio2 status");
                
//                OSAFlagSet(GpioFlgRef, 0x01, OSA_FLAG_OR);
                app_led_msgq_msg gpioMsg = {0};

                gpioMsg.msgId = LED_TASK_MSG_KEY3_MSG;
                send_task_send_msgq(&gpioMsg);
            }
        }
        lastKeyStatus2 = 1;    //20201016    
...
static void _gpiotimer_callback3(UINT32 tmrId)
{
    ...
    else
    {
        if(lastKeyStatus3 == 0)
        {
            ...
            uprintf("diff3:%d,elink_time3:%d",diff3,elink_time3);
            if(diff3 > 20)
            {
                //20201022
                uprintf("change gpio3 status");
                
//                OSAFlagSet(GpioFlgRef, 0x01, OSA_FLAG_OR);
                app_led_msgq_msg gpioMsg = {0};

                gpioMsg.msgId = LED_TASK_MSG_KEY4_MSG;
                send_task_send_msgq(&gpioMsg);
            }
        }
        lastKeyStatus3 = 1;    //20201016    
...
// End of file : am_key.c 2024-6-26 10:19:26 by: zhaoning 
