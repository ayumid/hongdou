...
/**
  * Function    : dtu_do_report_timer_stop
...
/**
  ...
  * Others      : 
  **/
void dtu_do_init_do(UINT8 channel, DTU_DO* doo)
{
    if(DTU_GPIO_CHNNEL_1 == channel)
    {
        //如果主动上报
        if(DTU_DO_REPORT_INITIATIVE == doo->type)
        {
            //启动主动上报定时器
            dtu_do_report_timer_start(DTU_TIME_INDEX_1, doo->interval);
        }
        else if(DTU_DO_REPORT_NONE == doo->type)
        {
            //关闭主动上报定时器
            OSATimerStop(dtu_do1_report_timer_ref);
        }
    ...
    else if(DTU_GPIO_CHNNEL_2 == channel)
    {
        ...
        {
            //启动主动上报定时器
            dtu_do_report_timer_start(DTU_TIME_INDEX_2, doo->interval);
        }
        else if(DTU_DO_REPORT_NONE == doo->type)
        {
            //关闭主动上报定时器
    ...
    else if(DTU_GPIO_CHNNEL_3 == channel)
    {
        ...
        {
            //启动主动上报定时器
            dtu_do_report_timer_start(DTU_TIME_INDEX_3, doo->interval);
        }
        else if(DTU_DO_REPORT_NONE == doo->type)
        {
            //关闭主动上报定时器
    ...
    else if(DTU_GPIO_CHNNEL_4 == channel)
    {
        ...
        {
            //启动主动上报定时器
            dtu_do_report_timer_start(DTU_TIME_INDEX_4, doo->interval);
        }
        else if(DTU_DO_REPORT_NONE == doo->type)
        {
            //关闭主动上报定时器
    ...
    else if(DTU_GPIO_CHNNEL_5 == channel)
    {
        ...
        {
            //启动主动上报定时器
            dtu_do_report_timer_start(DTU_TIME_INDEX_5, doo->interval);
        }
        else if(DTU_DO_REPORT_NONE == doo->type)
        {
            //关闭主动上报定时器
...
//    OSATimerStop(dtu_do5_timer_ref);
    ...
    //初始化DO状态
    if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_1].status == DTU_DO_OUT_STATUS_LAST)
    {
//        uprintf("%s init set restore:%d\n", channel, doo->do_out);
        GpioSetLevel(DTU_GPIO_DO_PIN1, dtu_file_ctx->doo.params[DTU_BUF_INDEX_1].out);
    }
    else if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_1].status == DTU_DO_OUT_STATUS_HIGH)
    {
...
//        uprintf("%s init set low\n", channel);
    ...
    if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_2].status == DTU_DO_OUT_STATUS_LAST)
    {
//        uprintf("%s init set restore:%d\n", channel, doo->do_out);
        GpioSetLevel(DTU_GPIO_DO_PIN2, dtu_file_ctx->doo.params[DTU_BUF_INDEX_2].out);
    }
    else if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_2].status == DTU_DO_OUT_STATUS_HIGH)
    {
//        uprintf("%s init set high\n", channel);
...
//        uprintf("%s init set low\n", channel);
    ...
    if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_3].status == DTU_DO_OUT_STATUS_LAST)
    {
//        uprintf("%s init set restore:%d\n", channel, doo->do_out);
        GpioSetLevel(DTU_GPIO_DO_PIN3, dtu_file_ctx->doo.params[DTU_BUF_INDEX_3].out);
    }
    else if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_3].status == DTU_DO_OUT_STATUS_HIGH)
    {
//        uprintf("%s init set high\n", channel);
...
//        uprintf("%s init set low\n", channel);
    ...
    if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_4].status == DTU_DO_OUT_STATUS_LAST)
    {
//        uprintf("%s init set restore:%d\n", channel, doo->do_out);
        GpioSetLevel(DTU_GPIO_DO_PIN4, dtu_file_ctx->doo.params[DTU_BUF_INDEX_4].out);
    }
    else if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_4].status == DTU_DO_OUT_STATUS_HIGH)
    {
//        uprintf("%s init set high\n", channel);
...
//        uprintf("%s init set low\n", channel);
    ...
    if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_5].status == DTU_DO_OUT_STATUS_LAST)
    {
//        uprintf("%s init set restore:%d\n", channel, doo->do_out);
        GpioSetLevel(DTU_GPIO_DO_PIN5, dtu_file_ctx->doo.params[DTU_BUF_INDEX_5].out);
    }
    else if(dtu_file_ctx->doo.params[DTU_BUF_INDEX_5].status == DTU_DO_OUT_STATUS_HIGH)
    {
//        uprintf("%s init set high\n", channel);
...
//        uprintf("%s init set low\n", channel);
    ...
    //初始化do上报状态
    dtu_do_init_do(DTU_GPIO_CHNNEL_1, &dtu_file_ctx->doo.params[DTU_BUF_INDEX_1]);
    dtu_do_init_do(DTU_GPIO_CHNNEL_2, &dtu_file_ctx->doo.params[DTU_BUF_INDEX_2]);
    dtu_do_init_do(DTU_GPIO_CHNNEL_3, &dtu_file_ctx->doo.params[DTU_BUF_INDEX_3]);
    dtu_do_init_do(DTU_GPIO_CHNNEL_4, &dtu_file_ctx->doo.params[DTU_BUF_INDEX_4]);
    dtu_do_init_do(DTU_GPIO_CHNNEL_5, &dtu_file_ctx->doo.params[DTU_BUF_INDEX_5]);
...
/**
  ...
  * Others      : 
  **/
void dtu_do_write_pin(UINT8 channel, UINT8 status)
{
    DTU_FILE_PARAM_T* dtu_file_ctx = NULL;
    ...
    if(DTU_GPIO_CHNNEL_1 == channel)
    {
        ...
        dtu_file_ctx->doo.params[channel].out = status;
        //如果输出保持时间不为0，而且当前是输出高电平
