...
//    uprintf("%s[%d]: DTU_DATA_MODE recv len: %d\n", __FUNCTION__, __LINE__, uartData->len);
    ...
    if(dtu_socket_group.fd)
    {
        ...
        dtu_format_send(data, send_size);
#else
        uprintf("%s[%d] senddata len: %d\n", __FUNCTION__, __LINE__, send_size);
        //数据发送到服务器
        dtu_socket_write((void *)data, send_size);
#endif /* ifdef DTU_TYPE_FORMAT_INCLUDE.2023-12-7 15:20:28 by: zhaoning */
...
/**
  ...
  * Others      : 
  **/
void dtu_sokcet_task_init(void)
{
    OSA_STATUS status = 0;
    
    //初始化下行消息队列
    status = OSAMsgQCreate(&dtu_socket_group.dtu_msgq_socket_recv, "dtu_msgq_socket_recv", sizeof(DTU_SOCKET_RECV_TYPE_T), 5, OS_FIFO);
