...
***********************************************************************/
static int dtu_atcmd_setmqttconn(char *atName, char *atLine, AtCmdResponse *resp);
static int dtu_atcmd_setmqttsub(char *atName, char *atLine, AtCmdResponse *resp);
static int dtu_atcmd_setmqttpub(char *atName, char *atLine, AtCmdResponse *resp);
static int dtu_atcmd_setmqttheart(char *atName, char *atLine, AtCmdResponse *resp);
static int dtu_atcmd_setmqttlink(char *atName, char *atLine, AtCmdResponse *resp);
...
static int AT_TRANS_CmdFunc_SETMQTTCONN(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp);//dmh
static int AT_TRANS_CmdFunc_SETMQTTSUB(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp);
static int AT_TRANS_CmdFunc_SETMQTTPUB(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp);
static int AT_TRANS_CmdFunc_SETMQTTHEART(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp);
static int AT_TRANS_CmdFunc_SETMQTTLINK(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp);
...
static ATCmdTable dtu_atcmd_table[] = {
    // add by dmh begin
    { "+SETMQTTCONN",dtu_atcmd_setmqttconn},
	{ "+SETMQTTSUB",dtu_atcmd_setmqttsub},
	{ "+SETMQTTPUB",dtu_atcmd_setmqttpub},
	{ "+SETMQTTHEART",dtu_atcmd_setmqttheart},
	{ "+SETMQTTLINK",dtu_atcmd_setmqttlink},
	{ "+HELP",dtu_atcmd_help},
...
#define SEND_MAX_BUFF 2048 //mqtt sendBuf length
...
static int uarthandle(MsgUartDataParam_sdk * uartData)
{
	...
	if(send_count >= SEND_MAX_BUFF)
	{
		if(MQTTIsConnected(client)){
			...
			result = mqtt_send(data,send_size);	
			free(data);
...
static void send_serial_data(MsgUartDataParam_sdk * uartData)
{
	...
	if(MQTTIsConnected(client)){
		...
		OSATimerStop(_cachetimer_ref);
		//huan cun
		memcpy(&(sendBuf[send_count]),(char *)uartData->UArgs,uartData->len);//dmh 20200713
	...
	if(serial_mode == DATA_MODE){
		if(MQTTIsConnected(client)){
			...
			if(strcmp(response->response,"ATO")==0 || strcmp(response->response,"ATO0")==0)
				...
				mqtt_send(resp_buffer, strlen(resp_buffer));
		}
	}else{
	    if (response->result == DSAT_OK) {
	        if (response->response && response->response[0]) {
...
void process_at_cmd_mode(const char *atCmdData, int data_len)
{
    ...
    while (data_len > 0) {
        ...
        case DSAT_CMD_PREP_STATE_FOUND_AT:
            ...
            if (cc != '\r') {
                if (cc == '=' || cc == '?') {
					if(strlen(atName)==0){//add by dmh 20200723
...
static int dtu_atcmd_setmqttconn(char *atName, char *atLine, AtCmdResponse *resp)
{
	...
	{
		result = AT_TRANS_CmdFunc_SETMQTTCONN(&pParam,atLine,resp);
		free_pParam(&pParam);
	}
	else{
		free_pParam(&pParam);
...
static int dtu_atcmd_setmqttsub(char *atName, char *atLine, AtCmdResponse *resp)
{
	...
	if(package_at_cmd(atName,atLine,&pParam)==0)
	{
		result = AT_TRANS_CmdFunc_SETMQTTSUB(&pParam,atLine,resp);
		free_pParam(&pParam);
	}
	else{
		free_pParam(&pParam);
...
static int dtu_atcmd_setmqttpub(char *atName, char *atLine, AtCmdResponse *resp)
{
	...
	if(package_at_cmd(atName,atLine,&pParam)==0)
	{
		result = AT_TRANS_CmdFunc_SETMQTTPUB(&pParam,atLine,resp);
		free_pParam(&pParam);
	}
	else{
		free_pParam(&pParam);
...
static int dtu_atcmd_setmqttheart(char *atName, char *atLine, AtCmdResponse *resp)
{
	...
	if(package_at_cmd(atName,atLine,&pParam)==0)
	{
		result = AT_TRANS_CmdFunc_SETMQTTHEART(&pParam,atLine,resp);
		free_pParam(&pParam);
	}
	else{
		free_pParam(&pParam);
...
static int dtu_atcmd_setmqttlink(char *atName, char *atLine, AtCmdResponse *resp)
{
	...
	if(package_at_cmd(atName,atLine,&pParam)==0)
	{
		result = AT_TRANS_CmdFunc_SETMQTTLINK(&pParam,atLine,resp);
		free_pParam(&pParam);
	}
	else{
		free_pParam(&pParam);
...
static int AT_TRANS_CmdFunc_SETMQTTCONN(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
    UINT8* cmdpw;
	UINT8 *ip;
	UINT8 port[10]={"\0"};
	...
	if (AT_CMD_SET == pParam->iType)
    {
        ...
        {			 		 
			...
			ip = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
			if ((!paramRet)||(strlen(ip) >= 64))
	        {
	        	return result;
	        }
			portInt = at_ParamUintInRange(pParam->params[index++], 0, 65535, &paramRet);
            if (!paramRet){
				return result;
			}
			sprintf(port,"%d",portInt);
			printf("port:%s",port);
			tmp_clientid = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
	        if (!paramRet || strlen(tmp_clientid) >500)
	        {
	          	return result;
	        }
	        aliveSeconds = (UINT16)at_ParamUint(pParam->params[index++], &paramRet);
	        if (!paramRet)
	        {
	            return result;
	        }
			...
			tmp_username = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
            if (!paramRet || strlen(tmp_username) >500)
            {
                return result;
            }
            tmp_password = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
            if (!paramRet || strlen(tmp_password) >500)
            {
                return result;
            }
			sprintf(transconf.ip,ip);
			sprintf(transconf.port,port);
			sprintf(transconf.clientid,tmp_clientid);
			transconf.keeplive = aliveSeconds;
			transconf.cleansession = cleansession;
			sprintf(transconf.username,tmp_username);
			sprintf(transconf.password,tmp_password);	
	...
	else if(pParam->iType == AT_CMD_READ)
	{
        ...
        sprintf(arrStr + strlen(arrStr), "\"%s\",%s,\"%s\",%d,%d,\"%s\",\"%s\"", transconf.ip,transconf.port,transconf.clientid,transconf.keeplive,transconf.cleansession,transconf.username,transconf.password);//dmh 2019-11-05
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
static int AT_TRANS_CmdFunc_SETMQTTSUB(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
	...
	if (AT_CMD_SET == pParam->iType)
    {
        if (pParam->paramCount == 3)
        {
			...
			subtopic = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
			if ((!paramRet)||(strlen(subtopic) >= 100))
	        {
	        	return result;
	        }
			subflag = at_ParamUintInRange(pParam->params[index++], 0, 1, &paramRet);
            if (!paramRet){
				return result;
			}
			
	        subqos = at_ParamUintInRange(pParam->params[index++], 0, 2, &paramRet);
	        if (!paramRet)
	        {
	           return result;
	        }
			
			sprintf(transconf.subtopic,subtopic);
			sprintf(transconf.subtopic1,"");
			sprintf(transconf.subtopic2,"");
			transconf.subflag=subflag;
			transconf.subqos=subqos;
		...
		if (pParam->paramCount == 4)
        {
			...
			subtopic1 = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
			...
			sprintf(transconf.subtopic,subtopic);
			sprintf(transconf.subtopic1,subtopic1);
			sprintf(transconf.subtopic2,"");
			transconf.subflag=subflag;
			transconf.subqos=subqos;
		...
		if (pParam->paramCount == 5)
        {
			...
			subtopic2 = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
			...
			sprintf(transconf.subtopic,subtopic);
			sprintf(transconf.subtopic1,subtopic1);
			sprintf(transconf.subtopic2,subtopic2);
			transconf.subflag=subflag;
			transconf.subqos=subqos;
	...
	else if(pParam->iType == AT_CMD_READ)
	{
        ...
        sprintf(arrStr + strlen(arrStr), "\"%s\",\"%s\",\"%s\",%d,%d", transconf.subtopic,transconf.subtopic1,transconf.subtopic2,transconf.subflag,transconf.subqos);//dmh 2019-11-05
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
static int AT_TRANS_CmdFunc_SETMQTTPUB(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
	...
	if (AT_CMD_SET == pParam->iType)
    {
        if (pParam->paramCount == 4)
        {
			...
			pubtopic = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
			if ((!paramRet)||(strlen(pubtopic) >= 100))
	        {
	        	return result;
	        }
			pubqos = at_ParamUintInRange(pParam->params[index++], 0, 1, &paramRet);
            if (!paramRet){
				return result;
			}
			...
			sprintf(transconf.pubtopic,pubtopic);
			transconf.pubqos=pubqos;
			transconf.duplicate = duplicate;
			transconf.retain = retain;
	...
	else if(pParam->iType == AT_CMD_READ)
	{
        ...
        sprintf(arrStr + strlen(arrStr), "\"%s\",%d,%d,%d", transconf.pubtopic,transconf.pubqos,transconf.duplicate,transconf.retain);//dmh 2019-11-05
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
static int AT_TRANS_CmdFunc_SETMQTTHEART(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
	...
	if (AT_CMD_SET == pParam->iType)
    {
    	if(pParam->paramCount == 1){
			heartflag=at_ParamUintInRange(pParam->params[index++], 0, 1, &paramRet);
			if (!paramRet){
				return result;
			}
			transconf.heartflag = heartflag;
			if(trans_conf_file_write(transconf)==0){
			 	char arrStr[100]="AT";
				sprintf(arrStr + strlen(arrStr), "%s",atLine);
			 	snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
        ...
        else if (pParam->paramCount == 3)
        {
	        ...
	        hearttime = at_ParamUintInRange(pParam->params[index++], 10, 3600, &paramRet);
	        if (!paramRet)
	        {
	            return result;
	        }
			heart = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
	...
	else if(pParam->iType == AT_CMD_READ)
	{
        ...
        sprintf(arrStr + strlen(arrStr), "%d,%d,\"%s\"", transconf.heartflag,transconf.hearttime,transconf.heart);//dmh 2019-11-05
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
static int AT_TRANS_CmdFunc_SETMQTTLINK(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
    ...
    {
    	if(pParam->paramCount == 1){
			linkflag=at_ParamUintInRange(pParam->params[index++], 0, 3, &paramRet);
			if (!paramRet){
				return result;
			}
			transconf.linkflag = linkflag;
			if(trans_conf_file_write(transconf)==0){
			 	char arrStr[100]="AT";
				sprintf(arrStr + strlen(arrStr), "%s",atLine);
			 	snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
        ...
        else if (pParam->paramCount == 2)
        {
			...
			link = (UINT8 *)at_ParamStr(pParam->params[index++], &paramRet);
	...
	else if(pParam->iType == AT_CMD_READ)
	{
        ...
        sprintf(arrStr + strlen(arrStr), "%d,\"%s\"", transconf.linkflag,transconf.link);//dmh 2019-11-05
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
static int AT_TRANS_CmdFunc_Query(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
	...
	if (pParam->iType == AT_CMD_EXE){
		...
		char arrStr[500] ={0};
		sprintf(arrStr + strlen(arrStr), "%s\"%s\",%s,\"%s\",%d,%d,\"%s\",\"%s\"","+SETMQTTCONN:", transconf.ip,transconf.port,transconf.clientid,transconf.keeplive,transconf.cleansession,transconf.username,transconf.password);//dmh 2019-11-05
		sprintf(arrStr + strlen(arrStr), "%s\"%s\",\"%s\",\"%s\",%d,%d","\r\n+SETMQTTSUB:",  transconf.subtopic,transconf.subtopic1,transconf.subtopic2,transconf.subflag,transconf.subqos);//dmh 2019-11-05
		sprintf(arrStr + strlen(arrStr), "%s\"%s\",%d,%d,%d","\r\n+SETMQTTPUB:", transconf.pubtopic,transconf.pubqos,transconf.duplicate,transconf.retain);//dmh 2019-11-05
		sprintf(arrStr + strlen(arrStr), "%s%d,%d,\"%s\"","\r\n+SETMQTTHEART:", transconf.heartflag,transconf.hearttime,transconf.heart);//dmh 2019-11-05
		sprintf(arrStr + strlen(arrStr), "%s%d,\"%s\"","\r\n+SETMQTTLINK:", transconf.linkflag,transconf.link);//dmh 2019-11-05
...
static int AT_TRANS_CmdFunc_Version(AT_CMD_PARA *pParam,char *atLine,AtCmdResponse *resp)
{
	...
	dsat_result_enum_type result = DSAT_ERROR;
	if (pParam->iType == AT_CMD_EXE){
		char arrStr[] ="AP4000MT_430E_MQTT_V2.0-20210819";
		snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
		result = DSAT_OK;
	}
	return result;
...
#endif /* ifdef INCLI_MOUDULES_MT_MQTT.2022-1-25 17:09:52 by: win */
