...
#define TM_TOPIC_DESIRED_PROPS_GET    ".property.desired.get" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_DESIRED_PROPS_DELETE ".property.desired.delete" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_SERVICE_INVOKE       ".service.%s.invoke" //Not supportted by OneJson-LwM2M
...
#define TM_TOPIC_EVENT_POST_REPLY           ".event.post.reply" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_DESIRED_PROPS_GET_REPLY    ".property.desired.get.reply" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_DESIRED_PROPS_DELETE_REPLY ".property.desired.delete.reply" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_SERVICE_INVOKE_REPLY       ".service.%s.reply"
#define TM_TOPIC_PACK_DATA_POST_REPLY       ".pack.post.reply" //Not supportted by OneJson-LwM2M
#define TM_TOPIC_HISTORY_DATA_POST_REPLY    ".history.post.reply" //Not supportted by OneJson-LwM2M
#else
...
{
    logd("construct_topic prefix %s, suffix %s\n",prefix, suffix);
...
{
#if defined(CONFIG_TM_MQTT) || defined(CONFIG_TM_NBIOT)
    uint8_t *topic = NULL;
#endif
...
static void tm_prop_get(uint8_t *payload, uint32_t payload_len)
{
    ...
    if (NULL == (reply_payload = osl_malloc(CONFIG_PACKET_PAYLOAD_LEN_MAX)))
    {
        loge("prop set error, malloc failed.");
        return;
    }
    osl_memset(reply_payload, 0, CONFIG_PACKET_PAYLOAD_LEN_MAX);
...
static int32_t tm_data_parse(const uint8_t *res_name, uint8_t *payload, uint32_t payload_len)
{
    ...
    {
        uint8_t        svc_id[32] = {0};
        const uint8_t *svc_id_ptr = res_name + osl_strlen(g_tm_obj.topic_prefix) + sizeof(".service.") - 1;
...
#elif defined(CONFIG_TM_NBIOT)
    return tm_lwm2m_init(tm_data_parse);
#endif
}
