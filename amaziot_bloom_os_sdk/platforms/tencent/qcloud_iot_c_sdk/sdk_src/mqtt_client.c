...
//    }
    ...
    mqtt_task_stack = malloc(MQTT_TASK_STACK_SIZE);
    
    ret = OSATaskCreate(&mqtt_task_ref, mqtt_task_stack, MQTT_TASK_STACK_SIZE, 100, "mqtt_yield_thread", _mqtt_yield_thread, pClient);
    if (ret) {
        Log_e("create mqtt yield thread fail: %d", ret);
        return QCLOUD_ERR_FAILURE;
    }
...
//    pClient->lock_generic = HAL_MutexCreate();
    OSAMutexCreate(&pClient->lock_generic, OS_PRIORITY);
    ...
    OSAMutexCreate(&pClient->lock_write_buf, OS_PRIORITY);
    if (pClient->lock_write_buf == NULL) {
        Log_e("create write buf lock failed.");
        goto error;
    }
    OSAMutexCreate(&pClient->lock_list_sub, OS_PRIORITY);
    if (pClient->lock_list_sub == NULL) {
        Log_e("create sub list lock failed.");
        goto error;
    }
    OSAMutexCreate(&pClient->lock_list_pub, OS_PRIORITY);
    if (pClient->lock_list_pub == NULL) {
