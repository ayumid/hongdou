                                    ...
                                    char* out, size_t* olen)
{
    ...
    if (rt != 0) {
        printf("atop_url_params_sign error:%d", rt);
        return rt;
    }
    printlen += sign_len;
    *olen = printlen;
                                            ...
                                            uint8_t* output, size_t* olen)
{
    ...
    if (rt != OPRT_OK) {
        printf("aes128_ecb_decode error:%d", rt);
        return rt;
    }
                                    ...
                                    uint8_t* output, size_t* olen)
{
    ...
    if (rt != OPRT_OK) {
        printf("base64 decode error:%d", rt);
    ...
    system_free(b64buffer);
    if (rt != OPRT_OK) {
        printf("atop_data_decrpyt error: %d", rt);
        return rt;
    }
    printf("result:\r\n%.*s", *olen, output);
                                            ...
                                            atop_base_response_t* response)
{
    ...
    if (input[ilen] != '\0') {
        printf("string length error ilen:%d, stlen:%d", ilen, strlen((char*)input));
    }
    ...
    if (!cJSON_HasObjectItem(root, "success")) {
        printf("not found json success key");
        cJSON_Delete(root);
    ...
    if (cJSON_GetObjectItem(root, "errorMsg")) {
        printf("errorMsg:%s", cJSON_GetObjectItem(root, "errorMsg")->valuestring);
    }
...
int atop_base_request(const atop_base_request_t* request, atop_base_response_t* response)
{
    ...
    if (NULL == response_buffer) {
        printf("response_buffer malloc fail");
        system_free(path_buffer);
        system_free(body_buffer);
        return OPRT_MALLOC_FAILED;
    }
    ...
    if (HTTP_CLIENT_SUCCESS != http_status) {
        printf("http_request_send error:%d", http_status);
    ...
    if (NULL == result_buffer) {
        printf("result_buffer malloc fail");
        system_free(response_buffer);
        return OPRT_MALLOC_FAILED;
    }
    ...
    printf("atop_response_decode error:%d, try parse the plaintext data.", rt);
