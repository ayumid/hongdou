...
/**
  * Function    : dtu_di1_report_timer_stop
...
/**
  * Function    : dtu_di2_report_timer_stop
  * Description : 停止DI2周期上报定时器
  * Input       : 
  *               
  * Output      : 
  * Return      : 
...
/**
  * Function    : dtu_di3_report_timer_stop
  * Description : 停止DI3周期上报定时器
  * Input       : 
  *               
  * Output      : 
  * Return      : 
...
/**
  * Function    : dtu_di4_report_timer_stop
  * Description : 停止DI4周期上报定时器
  * Input       : 
  *               
  * Output      : 
  * Return      : 
...
/**
  * Function    : dtu_di5_report_timer_stop
  * Description : 停止DI5周期上报定时器
  * Input       : 
  *               
  * Output      : 
  * Return      : 
...
static void dtu_gpio_timer_cbk1(UINT32 tmrId)
{
    ...
    if(DTU_DO_FLOW_DI_FORWARD == dtu_file_ctx->flow.do1_flow)
    {
        dtu_do_write_pin("DO1", p_smsg.status);
    }
    else if(DTU_DO_FLOW_DI_BACKWARD == dtu_file_ctx->flow.do1_flow)
    {
        dtu_do_write_pin("DO1", ~p_smsg.status);
    }
    uprintf("%s PIN1 status: %d \n", __FUNCTION__, p_smsg.status);
    if(DTU_DI_REPORT_TRIGGER == dtu_file_ctx->di.di1.di_type)
    {
        if(p_smsg.status == dtu_file_ctx->di.di1.di_edge)
        {
...
static void dtu_gpio_timer_cbk2(UINT32 tmrId)
{
    ...
    if(DTU_DO_FLOW_DI_FORWARD == dtu_file_ctx->flow.do2_flow)
    {
        dtu_do_write_pin("DO2", p_smsg.status);
    }
    else if(DTU_DO_FLOW_DI_BACKWARD == dtu_file_ctx->flow.do2_flow)
    {
        dtu_do_write_pin("DO2", ~p_smsg.status);
    }
    uprintf("%s PIN2 status: %d \n", __FUNCTION__, p_smsg.status);
    if(DTU_DI_REPORT_TRIGGER == dtu_file_ctx->di.di2.di_type)
    {
        if(p_smsg.status == dtu_file_ctx->di.di1.di_edge)
        {
            dtu_json_task_send_msgq(&p_smsg);
...
static void dtu_gpio_timer_cbk3(UINT32 tmrId)
{
    ...
    if(DTU_DO_FLOW_DI_FORWARD == dtu_file_ctx->flow.do3_flow)
    {
        dtu_do_write_pin("DO3", p_smsg.status);
    }
    else if(DTU_DO_FLOW_DI_BACKWARD == dtu_file_ctx->flow.do3_flow)
    {
        dtu_do_write_pin("DO3", ~p_smsg.status);
    }
    uprintf("%s PIN3 status: %d \n", __FUNCTION__, p_smsg.status);
    if(DTU_DI_REPORT_TRIGGER == dtu_file_ctx->di.di3.di_type)
    {
        if(p_smsg.status == dtu_file_ctx->di.di1.di_edge)
        {
            dtu_json_task_send_msgq(&p_smsg);
...
static void dtu_gpio_timer_cbk4(UINT32 tmrId)
{
    ...
    if(DTU_DO_FLOW_DI_FORWARD == dtu_file_ctx->flow.do4_flow)
    {
        dtu_do_write_pin("DO4", p_smsg.status);
    }
    else if(DTU_DO_FLOW_DI_BACKWARD == dtu_file_ctx->flow.do4_flow)
    {
        dtu_do_write_pin("DO4", ~p_smsg.status);
    }
    uprintf("%s PIN4 status: %d \n", __FUNCTION__, p_smsg.status);
    if(DTU_DI_REPORT_TRIGGER == dtu_file_ctx->di.di4.di_type)
    {
        if(p_smsg.status == dtu_file_ctx->di.di1.di_edge)
        {
            dtu_json_task_send_msgq(&p_smsg);
...
static void dtu_gpio_timer_cbk5(UINT32 tmrId)
{
    ...
    if(DTU_DO_FLOW_DI_FORWARD == dtu_file_ctx->flow.do5_flow)
    {
        dtu_do_write_pin("DO5", p_smsg.status);
    }
    else if(DTU_DO_FLOW_DI_BACKWARD == dtu_file_ctx->flow.do5_flow)
    {
        dtu_do_write_pin("DO5", ~p_smsg.status);
    }
    uprintf("%s PIN5 status: %d \n", __FUNCTION__, p_smsg.status);
    if(DTU_DI_REPORT_TRIGGER == dtu_file_ctx->di.di5.di_type)
    {
        if(p_smsg.status == dtu_file_ctx->di.di1.di_edge)
        {
            dtu_json_task_send_msgq(&p_smsg);
