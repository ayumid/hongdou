...
#define DTU_TASK_STACK_SIZE     1024 * 2
...
#if !defined (DTU_TYPE_EXSIM_GNSS) && !defined (DTU_TYPE_EXSIM)
...
void Phase2Inits_exit(void)
{
    ...
    status = OSAMsgQCreate(&dtu_uart_ctx->dtu_msgq_uart, "dtu_msgq_uart", sizeof(DTU_MSG_UART_DATA_PARAM_T), 500, OS_FIFO);
    DIAG_ASSERT(status == OS_SUCCESS);
    //初始化下行任务
    status = OSAMsgQCreate(&dtu_socket_ctx->dtu_msgq_socket_recv, "dtu_msgq_socket_recv", sizeof(DTU_SOCKET_RECV_TYPE_T), 500, OS_FIFO);
    DIAG_ASSERT(status == OS_SUCCESS);
    
#if defined (DTU_TYPE_EXSIM_GNSS) || defined (DTU_TYPE_5X6_GNSS) || defined (DTU_TYPE_3IN1_GNSS)
    //初始化gnss数据上报队列
    status = OSAMsgQCreate(&dtu_uart_ctx->dtu_msgq_uart4, "dtu_msgq_uart4", sizeof(DTU_MSG_UART_DATA_PARAM_T), 500, OS_FIFO);
    DIAG_ASSERT(status == OS_SUCCESS);
#endif
    //初始化网络连接情况超时检查定时器
    ...
    //下行数据接收线程
    sys_thread_new("dtu_sockrcv_thread", (lwip_thread_fn)dtu_sockrcv_thread, NULL, DEFAULT_THREAD_STACKSIZE, 161);
    //串口数据接收线程
    sys_thread_new("dtu_uart_data_recv_thread", (lwip_thread_fn)dtu_uart_data_recv_thread, NULL, DEFAULT_THREAD_STACKSIZE, 161);

#if defined (DTU_TYPE_EXSIM_GNSS) || defined (DTU_TYPE_5X6_GNSS) || defined (DTU_TYPE_3IN1_GNSS)
    //创建gnss串口4接收任务
    sys_thread_new("dtu_uart4_data_send_thread", (lwip_thread_fn)dtu_uart4_data_send_thread, NULL, DEFAULT_THREAD_STACKSIZE, 161);
#endif
...
//    printf("udp00");
    ...
    if(dtu_file_ctx->reg.linkflag == 1 || dtu_file_ctx->reg.linkflag == 3)
    {
        ...
        //发送数据到服务器
        if(0 != dtu_socket_write(dtu_socket_ctx->fd,(void *)data,link_size))
            goto PRO_START;
        //释放内存，置空
        if(NULL != hex_data)
        {
