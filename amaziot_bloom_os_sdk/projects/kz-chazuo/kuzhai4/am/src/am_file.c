...
void trans_conf_file_init(void)
{
    ...
    if (fd)
    {
      cprintf("FDI_fopen is OK!");
      transconf = trans_conf_file_read(fd);
    }
    else
    {
      cprintf("FDI_fopen is ERROR!");
...
int trans_conf_file_write(trans_conf transconf)
{
   ...
   if (fd != 0)
   {
        cprintf("debug> open file for write OK");
        FDI_fwrite(&transconf,sizeof(transconf), 1, fd);
           FDI_fclose(fd);
        cprintf("debug> trans_file write OK");
   }
   else
   {
        cprintf("debug> open file for write error");
        return -1;
   }
   return 0;
}
...
// End of file : am_file.h 2023-3-18 17:12:08 by: zhaoning 
