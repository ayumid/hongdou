...
//        cprintf("pool size: %d calloc size: %d", pool_size, _MAIN_TASK_STACK_SIZE);//pool size: 405120 calloc size: 65536(这个pool 上电后可能会变化，可能是360000+)
...
static void package_json_post_data()
{
    ...
    if(NULL != out)
    {
        cprintf("jsonData:%s",out);
//        cprintf("jsonData:%s",out + 70);
        sprintf(jsonStr, out);
        cJSON_free(out);
        out = NULL;
    }
...
static UINT32 parse_json_post_data(char *str)
{
    ...
    {
        cprintf("parse_json_post_data error");
    }
    if(NULL != hiDevice)
    {
...
static UINT32 parse_json_ls_getarea_data(char * str)
{
    ...
    {
        cprintf("parse_json_ls_getarea_data error");
    }
    if(NULL != hiDevice)
    {
        cJSON_Delete(hiDevice);
        ...
        if(pItemErr != NULL)
        {
            switch (pItemErr->valueint)
            {
                case 0:
                    ...
                    if(pItemdt != NULL)
                    {
//                        cprintf("pItem->valuestring:%s",cJSON_Print(pItemdt));
                        //GTM jiexi//2020-07-31T08:50:00.272Z
                        if(sscanf(pItemdt->valuestring,"%d-%d-%dT%d:%d:%d.%dZ",&year,&mon,&mday,&mhour,&min,&sec,&msec)>=6)
                        {
                            cprintf("jiexiOK!!!year:%d,mon:%d,day:%d,hour:%d,min:%d,sec:%d",year,mon,mday,mhour,min,sec);
                        ...
                        if(pItemPmtm != NULL)
                        {
                            ...
                            if(clocks > 0)
                            {
                                ...
                                for(i = 0; i < clocks; i++){
                                    ...
                                    if(clock != NULL)
                                    {
//                                        cprintf("clock:%s",cJSON_Print(clock));
                                        ...
                                        if(at != NULL)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    cprintf("jiexiOK!!!year:%d,mon:%d,day:%d,hour:%d,min:%d,sec:%d\n",year,mon,mday,mhour,min,sec);
...
//                        pItem = cJSON_GetObjectItem(hiDevice, "params");
                        ...
                        if(pItemPl != NULL)
                        {
//                            cprintf("pItem->valuestring:%s",cJSON_Print(pItemPl));
                            pulse0 = cJSON_GetArrayItem(pItemPl, 0);
                            if(pulse0 != NULL)
                            {
//                                cprintf("pulse0->valuestring:%s",cJSON_Print(pulse0));
                                pulse = cJSON_GetObjectItem(pulse0, "pulse");
                                if(pulse != NULL)
                                {
                                    cprintf("pulse->valuestring:%s",pulse->valuestring);
                            ...
                            if(pulse1 != NULL)
                            {
//                                cprintf("pulse1->valuestring:%s",cJSON_Print(pulse1));
                            ...
                            if(pulse2 != NULL)
                            {
//                                cprintf("pulse2->valuestring:%s",cJSON_Print(pulse2));
                            ...
                            if(pulse3 != NULL)
                            {
//                                cprintf("pulse3->valuestring:%s",cJSON_Print(pulse3));
...
//                        pItem = cJSON_GetObjectItem(hiDevice, "params");
                        ...
                        if(pItemlk != NULL)
                        {
//                            cprintf("pItem->valuestring:%s",cJSON_Print(pItemlk));
    ...
    {
        cprintf("parse_json_zs_register_data error");
    }
    if(NULL != hiDevice)
    {
        cJSON_Delete(hiDevice);
...
//        //cprintf("dmtest:hiDevice:%s",cJSON_Print(hiDevice));
//        memset(jsonStr,0,sizeof(jsonStr));
...
//    else
//    {
//        cprintf("ack_json_zs_updata_data error");
//    }
//    cJSON_Delete(hiDevice);
//    return ret;
//}
...
static UINT32 parse_json_zs_updata_data(char * str)
{
    ...
    if(hiDevice != NULL)
    {
        ...
        {
            //get switches
            //cprintf("pItem->valuestring:%s",cJSON_Print(pItem));
            pItemSw = cJSON_GetObjectItem(pItemPm, "switches");
            if(pItemSw != NULL)
            {
//                cprintf("pItem->valuestring:%s",cJSON_Print(pItemSw));
                ...
                for(i = 0; i < switchs; i++)
                {
                    ...
                    if(switch0 != NULL)
                    {
//                        cprintf("switch0->valuestring:%s",cJSON_Print(switch0));
...
//            pItem = cJSON_GetObjectItem(hiDevice, "params");
            ...
            if(pItemCfg != NULL)
            {
//                cprintf("pItem->valuestring:%s",cJSON_Print(pItemCfg));
                configure0 = cJSON_GetArrayItem(pItemCfg, 0);
                if(configure0 != NULL)
                {
//                    cprintf("configure0->valuestring:%s",cJSON_Print(configure0));
                    startup = cJSON_GetObjectItem(configure0, "startup");
                    if(startup != NULL)
                    {
                        cprintf("startup->valuestring:%s",startup->valuestring);
                ...
                if(configure1 != NULL)
                {
//                    cprintf("configure1->valuestring:%s",cJSON_Print(configure1));
                ...
                if(configure2 != NULL)
                {
//                    cprintf("configure2->valuestring:%s",cJSON_Print(configure2));
                ...
                if(configure3 != NULL)
                {
//                    cprintf("configure3->valuestring:%s",cJSON_Print(configure3));
...
//            pItem = cJSON_GetObjectItem(hiDevice, "params");
            ...
            if(pItemSo != NULL)
            {
//                cprintf("pItem->valuestring:%s",cJSON_Print(pItemSo));
...
//                OSAFlagSet(GpioFlgRef, 0x01, OSA_FLAG_OR);
    ...
    {
        cprintf("parse_json_zs_updata_data error");
    }
    if(NULL != hiDevice)
    {
        cJSON_Delete(hiDevice);
...
static void up_json_zs_updata_data()
{
    ...
    cprintf("rssiStr:%s",rssiStr);
    //get IMEI
    get_IMEI(imeiBuf);
    cprintf("imeiBuf:%s",imeiBuf);
    //get IMSI
    get_IMSI(imsiBuf);
    cprintf("imsiBuf:%s",imsiBuf);
    //get ICCID
    get_ICCID(iccidBuf);
    cprintf("iccidBuf:%s",iccidBuf);
...
static void up_json_zs_updata_switches()
{
    ...
    if(NULL != out)
    {
        ...
        if(ret <= 0)
        {
            cprintf("websocket send switches err");
        } 
        else
            cprintf("websocket send switches ok");
    }
...
static void up_json_zs_updata_alarm()
{
    ...
    if(NULL != out)
    {
        ...
        if(ret <= 0)
        {
            cprintf("websocket send alarm err");
        } 
        else
            cprintf("websocket send alarm ok");
    }
...
//                    cprintf("pItem->valuestring:%s",cJSON_Print(pItem));
    ...
    {
        cprintf("parse_json_zs_notify_data error");
    }
    if(NULL != hiDevice)
    {
        cJSON_Delete(hiDevice);
...
//    else
//    {
//        cprintf("ack_json_zs_notify_data error");
//    }
//    cJSON_Delete(hiDevice);
//    return ret;
//}
...
//            cprintf("pItem->valuestring:%s",cJSON_Print(pItem));
        ...
        if(params != NULL)
        {
//            cprintf("pItem->valuestring:%s",cJSON_Print(params));
                ...
                if(strcmp(pItemMd->valuestring,transconf.device_model) == 0)
                {
                    ...
                    if(bin != NULL)
                    {
                        ...
                        for(i=0;i<bins;i++){
                            ...
                            cprintf("pItem->valuestring:%s",cJSON_Print(bin));
                            url = cJSON_GetObjectItem(bin, "downloadUrl");
                            if(url!=NULL)
                                cprintf("pItem->valuestring:%s",cJSON_Print(url));
                            name = cJSON_GetObjectItem(bin, "name");
                            if(name!=NULL)
                                cprintf("pItem->valuestring:%s",cJSON_Print(name));
                            digest = cJSON_GetObjectItem(bin, "digest");
                            if(digest!=NULL)
                                cprintf("pItem->valuestring:%s",cJSON_Print(digest));
                        }
                        */
...
//                                cprintf("pItem->valuestring:%s",cJSON_Print(url));
    ...
    {
        cprintf("parse_json_zs_upgrade_data error");
    }
    if(NULL != hiDevice)
    {
        cJSON_Delete(hiDevice);
...
static void sw_http_recv_handler(void *handle, const http_recv_t *packet, void *userdata)
{
    ...
    switch (packet->type) {
        case HTTPRECV_STATUS_CODE:
            ...
            cprintf("status code: %d", packet->data.status_code.code);
            
        break;

        case HTTPRECV_HEADER:
            //SDK收到HTTP报文时, 会通过这个用户回调打印HTTP首部, 如Content-Length等
            //cprintf("key: %s, value: %s", packet->data.header.key, packet->data.header.value);
            
        break;

        case HTTPRECV_BODY:
            //处理云平台的HTTP回应报文
            //cprintf("len = %d, %s", packet->data.body.len,packet->data.body.buffer);
            ...
            if(NULL != body)
            {
                ...
                cprintf("len = %d, body: %s", packet->data.body.len, body);
...
PRO_START:
    ...
    if (http_handle == NULL)
    {
        cprintf("%s[%d], res: %d", __FUNCTION__, __LINE__, res);
        goto clean;
    }

    if(strlen(transconf.zhengshi)== 0)
    {
        cprintf("linshi:%s",transconf.linshi);
...
clean:
    ...
    cprintf("http clean the %d power of 2", errorCnt);
...
static int wait_dev_connectDomain(void)
{
    cprintf("transconf0.ip:%s",transconf0.ip);
    cprintf("transconf0.port:%d",transconf0.port);
    int errorCnt = 0;
wait_reg:
    if(((fd = webSocket_clientLinkToServer(transconf0.ip, transconf0.port, "api/ws", 1)) <= 0))
    {
        cprintf("client link to server failed !fd:%d",fd);
        errorCnt++;
        if(errorCnt > 3)
        {
            return -1;
    ...
    cprintf("fd:%d",fd);
    cprintf("websocket connect success!");
    return 0;
}
...
PRO_START:
    ...
    cprintf("%s-%d",__FUNCTION__,__LINE__);
    ...
    while(1)
    {
        ...
        cprintf("ssl_read buf: %s ret:%d fd:%d",buf, ret, fd);
...
static void send_task(void * argv)
{
    ...
    while(1)
    {
        ...
        {
//            cprintf("%s, msgid=%d", __FUNCTION__, sdkMsg.msgId);
            switch(sdkMsg.msgId)
            {
...
int send_task_send_msgq(app_msgq_msg * msg)
{
    ...
    if (status != OS_SUCCESS)
    {
        cprintf("%s, OSAMsgQSend lose, msg->id=%d, status=%d", __FUNCTION__, msg->msgId, status);
        ret = -1;
    }    
...
static void _hearttimer_callback(UINT32 tmrId)
{
//    cprintf("asr_test  _haerttimer_callback");
...
//        {
            if(tcpType > TCP_INITIAL)
            {
                ...
                if(ret <= 0) 
                    cprintf("websocket send err");
                else
                    cprintf("websocket send buf:%s",buf);
            }
//            if(tcpType > TCP_INITIAL)
//            {
//                cprintf("heart data");
...
//                    cprintf("websocket send err");
//                else
//                    cprintf("websocket send buf:%s %s",buf, memremain);
//            }
//        }
//    }
}
...
//            cprintf("rssi:%d abs:%d",rssi, csqGrade);
            if(csqGrade > 10)
            {
...
static void pluse_thread0(void * argv)
{
    ...
    while(1)
    {
        ...
        if(flag_value & 0x01)
        {
            cprintf("transconf.pulsetime0:%d",transconf.pulsetime0);
            if(transconf.pulsetime0 > 0)
            {
...
static void pluse_thread1(void * argv)
{
    ...
    while(1)
    {
        ...
        if(flag_value & 0x01)
        {
            cprintf("transconf.pulsetime1:%d",transconf.pulsetime1);
            if(transconf.pulsetime1 > 0)
            {
...
static void pluse_thread2(void * argv)
{
    ...
    while(1)
    {
        ...
        if(flag_value & 0x01)
        {
            cprintf("transconf.pulsetime2:%d",transconf.pulsetime2);
            if(transconf.pulsetime2 > 0)
            {
...
static void pluse_thread3(void * argv)
{
    ...
    while(1)
    {
        ...
        if(flag_value & 0x01)
        {
            cprintf("transconf.pulsetime3:%d",transconf.pulsetime3);
            if(transconf.pulsetime3 > 0)
            {
...
static void sw_fotaresultCallback(UINT32 result)
{
    cprintf("%s: result = %ld",__FUNCTION__, result);
    if(result == FOTA_SUCCESS)
    {
        cprintf("Down Bin Ok! Moudle Rebooting For Update...");
        sleep(1);
        PM812_SW_RESET();// restart will update
    }
    else
    {
        cprintf("Fota Error,Please try again!");
        MoudleLedStatus = NORMAL;
...
{
    cprintf("fota_start");
    cprintf("downloadUrl:%s",transconf0.downloadUrl);
...
static uint8_t GetWakeUpTimePoint2(AlarmClock *alarm,struct data_time_str *p_data)
{
    if(alarm->type == 1)
    {
        ...
        && (p_data->sec == 0))
        {
            cprintf("%04d-%02d-%02d  %02d:%02d:%02d   week:%d",
...
/*function for cprintf time*/
static uint8_t print_time_test(struct data_time_str *p_data)
{
    cprintf("%s[%d]%04d-%02d-%02d  %02d:%02d:%02d  week:%d", __FUNCTION__, __LINE__,
            p_data->year,p_data->month,p_data->day,
            p_data->hour,p_data->min,p_data->sec,p_data->week);
