...
{
    char *sendData = NULL;
    printf("%s[%d]:toUart=%s, length=%d", __FUNCTION__, __LINE__, toUart, length);
    ...
    printf("%s[%d]:send to uart data=%s, length=%d", __FUNCTION__, __LINE__, sendData, length);    
    UART_SEND_DATA((UINT8 *)sendData, length);
    printf("%s[%d]:sendData==NULL = %d", __FUNCTION__, __LINE__, (sendData==NULL));
    if (sendData)
        free(sendData);
}
...
static void uartdata_thread(void)
{
    ...
    while (1)
    {
        ...
        if (status == OS_SUCCESS)
        {
            if (uart_temp.UArgs)
            {
                printf("%s[%d]: uart_temp len:%d, data:%s", __FUNCTION__, __LINE__, uart_temp.len, (char *)(uart_temp.UArgs));
...
static void send_serial_data(MsgUartDataParam_sdk * uartData)
{
    printf("%s[%d]: DATA_MODE recvdata: %s", __FUNCTION__, __LINE__, uartData->UArgs);
}
...
static void handle_serial_data(MsgUartDataParam_sdk *uartData)
{
    ...
    else if(serial_mode == AT_MODE)
    {
        ...
        else if(strstr((const char*)uartData->UArgs,"\r\n"))
        {
            ...
            printf("[atcmd_test]: send AT command uargs:%s,len:%d",tmp, strlen(tmp) );
...
static void process_at_cmd_mode(const char *atCmdData, int data_len)
{
    ...
    buf_ptr = atCmdData;
    printf("%s[%d]: AT_MODE recv ATCMD: %s", __FUNCTION__, __LINE__, buf_ptr);
    //memset(tmp, 0, sizeof(tmp));//for 32bit error
    while (data_len > 0) {
        cc = *buf_ptr++;
        data_len--;
        ...
        case DSAT_CMD_PREP_STATE_FOUND_AT:
            ...
            if (cc != '\r') {
                if (cc == '=' || cc == '?') {
                    printf("sio_info_ptr->cmd_line_buffer:%s", sio_info_ptr->cmd_line_buffer);
                    strncpy(atName, sio_info_ptr->cmd_line_buffer, step);
                    atName[step-1] = '\0';
                    printf("atName is %s", atName);
                }
                ...
                *sio_info_ptr->build_cmd_ptr = '\0';
                ...
                if (!strlen(atName)) {
                    ...
                    atName[step-1] = '\0';
                    printf("[%d]: atName is %s", __LINE__, atName);
                }

                printf("%s[%d]: cmd_line_buffer=%s", __FUNCTION__, __LINE__, sio_info_ptr->cmd_line_buffer);
...
//            printf("%s[%d]arrStr==>%s %d", __FUNCTION__, __LINE__, arrStr, strlen(arrStr));
            snprintf(resp->response, strlen(arrStr)+1, "%s", arrStr);
            result = DSAT_OK;
         }
    }
...
//    printf("%s[%d]mmRspBuf==>%s %d", __FUNCTION__, __LINE__, mmRspBuf, strlen(mmRspBuf));
