...
//    printf("enter %s ...", __FUNCTION__);
    ...
    printf("webSocket_matchShakeKey:myKey:%s,acceptKey:%s",myKey,acceptKey);
...
//    *package = 0x00;
    ...
    unsigned int i = 0;
    unsigned int len = 0;
    printf("packageMaxLen:%d,type:%d,dataLen:%d,isMask:%d",packageMaxLen,type,dataLen,isMask);
    //147 -19 19 1
    if (packageMaxLen < 2)
        return -1;
...
int webSocket_clientLinkToServer(char *ip, int port, char *interface_path, int sslFlag)
{
    ...
    if ((report_addr.sin_addr.s_addr = inet_addr(ip)) == INADDR_NONE) 
    {
        ...
        printf("webSocket_clientLinkToServer : Host(%s) to Ip(%s)", ip, tempIp);
#endif
    }
...
//    ioctlsocket(fd, FIONBIO, &on);
    ...
    while (connect(fd, (struct sockaddr *)&report_addr, sizeof(struct sockaddr)) == -1)
    {
        ...
        {
            printf("webSocket_login : cannot connect to %s:%d ! %d", ip, port, timeOut);
            close(fd);
            return -timeOut;
        }
    ...
    printf("web_ssl_client_init sslFlag %d, %d", sslFlag, (web_ssl_c == NULL));    
...
//
    printf("web_ssl_client_init %d",(web_ssl_c == NULL));
    ...
    printf("loginBuf %d, %s", strlen((char *)loginBuf), loginBuf);
...
//            printf("login_recv : %d / %dms\r\n%s", ret, timeOut, recBuf);
            ...
            if (strncmp((const char *)recBuf, (const char *)"HTTP", strlen((const char *)"HTTP")) == 0) 
            {
                    ...
                    ||((p = (unsigned char *)strstr((const char *)recBuf, (const char *)"sec-websocket-accept: ")) != NULL))
                {
                    ...
                    printf("sec-websocket-accept:%s", p);
...
#ifdef WEBSOCKET_DEBUG
    unsigned int i = 0;
    printf("webSocket_send : %d", dataLen);
#endif
...
#ifdef WEBSOCKET_DEBUG
    printf("webSocket_send : %d", retLen);
    for (i = 0; i < retLen; i++)
...
int webSocket_recv(int fd, char *data, int dataMaxLen, WebsocketData_Type *dataType)
{
    ...
    printf("%s[%d],recvBuf:%p %ld", __FUNCTION__, __LINE__,recvBuf, OsaGetDefaultMemPoolFreeSize());
    if(NULL == recvBuf)
    {
    ...
    printf("ret:%d\n",ret);
    if (ret == dataMaxLen)
        printf("webSocket_recv : warning !! recv buff too large !! (recv/%d)", ret);
    if (ret > 0)
    {
        ...
        printf("dataLen:%d,packageLen:%d,headLen:%d,dpRet:%d",ret,retLen,retHeadLen,dpRet);
        
        if (retLen > 0)
        {
            printf("%s[%d], cpy %p %p %d", __FUNCTION__, __LINE__,data, webSocketPackage, retLen);
            memcpy(data, webSocketPackage, retLen);
            retFinal = retLen;
        }
    }
    if (NULL != recvBuf)
    {printf("%s[%d], freercvbuf:%p", __FUNCTION__, __LINE__,recvBuf);
        free(recvBuf);printf("%s[%d], freercvbuf:%p", __FUNCTION__, __LINE__,recvBuf);
        recvBuf = NULL;
    }
    if (NULL != webSocketPackage)
    {printf("%s[%d], freewebSocketPackage:%p", __FUNCTION__, __LINE__,webSocketPackage);
        free(webSocketPackage);printf("%s[%d], freewebSocketPackage:%p", __FUNCTION__, __LINE__,webSocketPackage);
        webSocketPackage = NULL;
    }
    if (NULL != dataType)
    {
