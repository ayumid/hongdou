...
// debug uart log
#define sample_w25q128_uart_printf(fmt, args...) \
    do                                \
    {                                 \
        sdklogConfig(1);              \
...
// CATStudio usb log
#define sample_w25q128_catstudio_printf(fmt, args...) \
    do                                 \
    {                                  \
        sdklogConfig(1);               \
        sdkLogPrintf(fmt, ##args);     \
...
#define sample_w25q128_sleep(x) OSATaskSleep((x)*200) // second

#define SAMPLE_W25Q128_STACK_SIZE           2048

// Private variables ------------------------------------------------------------

static void *sample_w25q128_task_stack = NULL;

static OSTaskRef sample_w25q128_task_ref = NULL;
...
//    }
    ...
    {
        /* 读取错误处理 */
        sample_w25q128_uart_printf("SPI read-write Error, please check the connection between MCU and SPI Flash\n");
    }
    else
    {
        //读取成功处理
...
//        sample_w25q128_catstudio_printf("ql_spi_write_read ret %d, readBuf %02X,%02X,%02X\n", ret, readBuf[0], readBuf[1], readBuf[2]);
//        ret = ql_spi_write(QL_SPI_PORT0, writeBuf, 1);
//        sample_w25q128_catstudio_printf("ql_spi_write_read ret %d, readBuf %02X\n", ret, writeBuf[0]);
