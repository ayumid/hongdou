...
int32_t _tls_network_establish(void *handle)
{
    ...
    if (adapter_handle->cred->ssl_verify_type == MBEDTLS_SSL_VERIFY_NONE) {
        mbedtls_ssl_conf_authmode(&adapter_handle->mbedtls.ssl_config, MBEDTLS_SSL_VERIFY_NONE);        
    }else if (adapter_handle->cred->ssl_verify_type == MBEDTLS_SSL_VERIFY_OPTIONAL || adapter_handle->cred->ssl_verify_type == MBEDTLS_SSL_VERIFY_REQUIRED) {
        ...
        if (adapter_handle->cred->x509_client_cert != NULL && adapter_handle->cred->x509_client_cert_len > 0 &&
            ...
            res = mbedtls_pk_parse_key(&adapter_handle->mbedtls.x509_client_pk,
                                       ...
                                       (size_t)adapter_handle->cred->x509_client_privkey_len + 1, NULL, 0, NULL, NULL);
            if (res < 0) {
...
void *adapter_network_init(void)
{
    ...
    core_log(g_origin_portfile, STATE_ADAPTER_COMMON, "adapter_network_init\r\n");
...
int32_t adapter_network_deinit(void **handle)
{
    ...
    if (adapter_handle->cred != NULL &&(adapter_handle->cred->ssl_verify_type == MBEDTLS_SSL_VERIFY_OPTIONAL || adapter_handle->cred->ssl_verify_type == MBEDTLS_SSL_VERIFY_REQUIRED)) {
...
aiot_sysdep_portfile_t *aiot_sysdep_get_adapter_portfile(aiot_sysdep_portfile_t *portfile, int tls)
{
    if (portfile == NULL) {
        return NULL;
    }
