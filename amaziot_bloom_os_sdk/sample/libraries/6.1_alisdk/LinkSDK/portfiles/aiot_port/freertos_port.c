...
#include "debug_log.h"
#undef printf
#define printf     catstudio_network_printf

#define CORE_RTOS_MS_ONE_TICK              (5) /*one tick denote 5ms*/
#define CORE_RTOS_TICK_PER_SECOND          (200) /*200 tick for 1s*/
...
uint64_t core_sysdep_time(void)
{
    ...
    return (uint64_t)(OSAGetTicks() * CORE_RTOS_MS_ONE_TICK);
}
...
void core_sysdep_sleep(uint64_t time_ms)
{
    //vTaskDelay(pdMS_TO_TICKS(time_ms));
       ...
       uint32_t ms = (uint32_t)time_ms;
    ms = (ms <= CORE_RTOS_MS_ONE_TICK ? 1 : (ms / CORE_RTOS_MS_ONE_TICK));
    OSATaskSleep(ms); /*tick*/  
}
...
void *core_sysdep_mutex_init(void)
{
    ...
    void *mutexRef;
    OSA_STATUS status;
    status = OSAMutexCreate(&mutexRef, OS_PRIORITY);
...
void core_sysdep_mutex_lock(void *mutex)
{
    ...
    OSA_STATUS status;
    status = OSAMutexLock(mutex, OSA_SUSPEND);
    ASSERT(status == OS_SUCCESS);
}
