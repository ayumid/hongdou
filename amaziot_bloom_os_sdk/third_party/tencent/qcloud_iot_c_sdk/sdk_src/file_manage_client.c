...
/*====================funtion ===================*/
char *tencent_strdup(const char *src)
{
...
#define COS_REPLY_TIMEOUT_MS (500)
    ...
    fptr = FDI_Open(info->file_name, "rb");
    if (NULL == fptr) {
                                ...
                                char *file_type)
{
    ...
    char *            msg_reported = NULL;
    int               ret          = QCLOUD_RET_SUCCESS;
    char *file_name_post = NULL;

    pHandle->request_id++;

    FilePostInfo *info = (FilePostInfo *)HAL_Malloc(sizeof(FilePostInfo));
    info->request_id   = pHandle->request_id;
    info->file_name    = tencent_strdup(file_name);
    info->file_version = tencent_strdup(file_version);
    info->file_type    = tencent_strdup(file_type);
    ...
    file_name_post = strrchr(file_name, '/');
    if (!file_name_post) {
