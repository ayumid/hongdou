                                ...
                                HTTPResponse_t * response)
{
    ...
    if( httpStatus != HTTPSuccess ) {
        printf("HTTP header error:%d", httpStatus);
    ...
    if( httpStatus != HTTPSuccess ) {
        printf( "Failed to send HTTP %.*s request to %.*s%.*s: Error=%s.",
                    ( int32_t ) requestInfo->methodLen, requestInfo->pMethod,
                    ( int32_t ) requestInfo->hostLen, requestInfo->pHost,
                    ( int32_t ) requestInfo->pathLen, requestInfo->pPath,
                                          ...
                                          http_client_response_t* response)
{
    ...
    if (OPRT_OK != rt) {
        printf("network_tls_init fail:%d", rt);
        return rt;
    }
    ...
    if (OPRT_OK != rt) {
        printf("network_tls_connect fail:%d", rt);
        network_tls_disconnect(&network);
        network_tls_destroy(&network);
        return rt;
    }
    printf("tls connencted!");
    ...
    if (OPRT_OK != rt) {
        printf("http_request_send error:%d", rt);
        return rt;
    }
