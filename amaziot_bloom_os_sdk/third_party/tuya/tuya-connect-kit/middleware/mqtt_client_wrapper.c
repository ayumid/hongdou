                                        ...
                                        struct MQTTDeserializedInfo* pDeserializedInfo )
{
    ...
    if( ( pPacketInfo->type & 0xF0U ) == MQTT_PACKET_TYPE_PUBLISH ) {
        ...
        case MQTT_PACKET_TYPE_SUBACK:
            printf("MQTT_PACKET_TYPE_SUBACK id:%d", msgid);
        ...
        case MQTT_PACKET_TYPE_UNSUBACK:
            printf("MQTT_PACKET_TYPE_UNSUBACK id:%d", msgid);
        ...
        case MQTT_PACKET_TYPE_PUBACK:
            printf("MQTT_PACKET_TYPE_PUBACK id:%d", msgid);
        ...
        default:
            printf("type:0x%02x, id:%d", pPacketInfo->type, msgid);
        }
    }
}
...
mqtt_client_status_t mqtt_client_init(void* client, const mqtt_client_config_t* config)
{
	...
	if (OPRT_OK != ret) {
		printf("network_tls_init fail:%d", ret);
    ...
    if( mqtt_status != MQTTSuccess ) {
        printf( "MQTT init failed: Status = %s.", MQTT_Status_strerror( mqtt_status ) );
        context->network.destroy(&context->network);
		return (mqtt_client_status_t)OPRT_COM_ERROR;
    }
...
mqtt_client_status_t mqtt_client_connect(void* client)
{
    ...
    if (MQTTSuccess != mqtt_status) {
        printf("mqtt connect err: %s(%d)", MQTT_Status_strerror(mqtt_status), mqtt_status);
        context->network.disconnect(&context->network);
...
mqtt_client_status_t mqtt_client_disconnect(void* client)
{
    ...
    if (MQTTSuccess != mqtt_status) {
        printf("mqtt disconnect err: %s(%d)", MQTT_Status_strerror(mqtt_status), mqtt_status);
    }
...
uint16_t mqtt_client_subscribe(void* client, const char* topic, uint8_t qos)
{
    ...
    if( mqtt_status != MQTTSuccess ) {
        printf( "Failed to send SUBSCRIBE packet to broker with error = %s.", MQTT_Status_strerror( mqtt_status ) );
        return 0;
    }
...
mqtt_client_status_t mqtt_client_yield(void* client)
{
    ...
    if( mqtt_status != MQTTSuccess ) {
        printf("MQTT_ProcessLoop returned with status = %s.", MQTT_Status_strerror( mqtt_status ));
