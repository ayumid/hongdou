...
int network_tls_connect(NetworkContext_t *pNetwork, const TLSConnectParams *params)
{
	...
	if(ret < 0) {
		printf(" failed! mbedtls_x509_crt_parse returned -0x%x while parsing root cert", -ret);
		return OPRT_MID_TLS_X509_ROOT_CRT_PARSE_ERROR;
	}
	printf("ok (%d skipped)", ret);
	...
	if (pNetwork->tlsConnectParams.client_cert && pNetwork->tlsConnectParams.client_key) {
		...
		if(ret != 0) {
			printf("failed! mbedtls_x509_crt_parse returned -0x%x while parsing device cert", -ret);
		...
		if(ret != 0) {
			printf("failed! mbedtls_pk_parse_key returned -0x%x while parsing private key", -ret);
			mbedtls_x509_crt_free(&(tlsDataParams->cacert));
	...
	printf("Connecting to %s/%s...", pNetwork->tlsConnectParams.host, portBuffer);
	if((ret = mbedtls_net_connect(&(tlsDataParams->server_fd), 
								  pNetwork->tlsConnectParams.host,
								  portBuffer, MBEDTLS_NET_PROTO_TCP)) != 0) {
		printf("failed! mbedtls_net_connect returned -0x%x", -ret);
		mbedtls_x509_crt_free(&(tlsDataParams->cacert));
		switch(ret) {
	...
	if((ret = mbedtls_ssl_config_defaults(&(tlsDataParams->conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
		...
		printf("failed! mbedtls_ssl_config_defaults returned -0x%x", -ret);
		mbedtls_x509_crt_free(&(tlsDataParams->cacert));
		return OPRT_MID_TLS_CONNECTION_ERROR;
	}
	...
	printf("SSL state connect: %d ", tlsDataParams->ssl.state);
