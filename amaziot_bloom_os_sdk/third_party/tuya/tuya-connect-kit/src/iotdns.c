...
static int iotdns_response_decode(const uint8_t* input, size_t ilen, tuya_endpoint_t* endport)
{
    ...
    printf("httpsSelfUrl:%s", httpsSelfUrl);
    printf("mqttsSelfUrl:%s", mqttsSelfUrl);

    /* ATOP url decode */
    int port = 443;
    sscanf(httpsSelfUrl, "https://%99[^/]%99[^\n]", endport->atop.host, endport->atop.path);
    endport->atop.port = (uint16_t)port;
    printf("endport->atop.host = \"%s\"", endport->atop.host);
    printf("endport->atop.port = %d", endport->atop.port);
    printf("endport->atop.path = \"%s\"", endport->atop.path);

    /* MQTT host decode */
    sscanf(mqttsSelfUrl, "%99[^:]:%99d[^\n]", endport->mqtt.host, &port);
    endport->mqtt.port = (uint16_t)port;
    printf("endport->mqtt.host = \"%s\"", endport->mqtt.host);
    printf("endport->mqtt.port = %d", endport->mqtt.port);
