...
static int activated_data_read(const char* storage_key, tuya_activated_data_t* out)
{
    ...
    if (NULL == readbuf) {
        printf("activate_string malloc fail.");
        return rt;
    }
    ...
    if (OPRT_OK != rt) {
        printf("activate config not found:%d", rt);
        system_free(readbuf);
        return rt;
    }
    ...
    system_free(readbuf);
    if (OPRT_OK != rt) {
        printf("activate_json_string_parse fail:%d", rt);
        return rt;
    }

    /* Dump info */
    printf("devId: %s", out->devid);
    printf("secKey: %s", out->seckey);
    printf("localKey: %s", out->localkey);
...
static int activate_response_parse(atop_base_response_t* response)
{
    ...
    if (ret != OPRT_OK) {
        printf("activate data save error:%d", ret);
    ...
    printf("result len %d :%s", (int)strlen(result_string), result_string);
    ...
    if(cJSON_GetObjectItem(result_root,"resetFactory") != NULL) {
        ...
        printf("cloud_reset:%d", cloud_reset_factory);
        //目前只有判断APP恢复出厂模式,但是本地简单移除配网信息,那么告知用户
        if(cloud_reset_factory == TRUE) {
            printf("remote is reset factory and local is not,reset factory again.");
...
static int client_activate_process(tuya_iot_client_t* client, const char* token)
{
    ...
    if (OPRT_OK != rt) {
        printf("activate_response_parse error:%d", rt);
        return rt;
    }
...
static void mqtt_service_reset_cmd_on(tuya_protocol_event_t* ev)
{
    ...
    if (NULL == cJSON_GetObjectItem(data, "gwId")) {
        printf("not found gwId");
    }

    printf("Reset id:%s", cJSON_GetObjectItem(data, "gwId")->valuestring);
    ...
    if (cJSON_GetObjectItem(ev->root_json, "type") && \
        ...
        printf("cmd is reset factory, ungister");
...
{
    tuya_iot_client_t* client = ev->user_data;
    printf("mqtt_atop_dp_cache_notify_cb");
...
static void mqtt_service_upgrade_notify_on(tuya_mqtt_event_t* ev)
{
    ...
    if (rt != OPRT_OK) {
        printf("upgrade info get error:%d", rt);
        return;
    }
}
...
static int run_state_mqtt_connect_start(tuya_iot_client_t* client)
{
    ...
    if (OPRT_OK != rt) {
        printf("tuya mqtt start error:%d", rt);
        return rt;
    }
...
int tuya_iot_init(tuya_iot_client_t* client, const tuya_iot_config_t* config)
{
    ...
    /* Config param dump */
    printf("software_ver:%s", client->config.software_ver);
    printf("productkey:%s", client->config.productkey);
    printf("uuid:%s", client->config.uuid);
    printf("authkey:%s", client->config.authkey);
...
int tuya_iot_yield(tuya_iot_client_t* client)
{
    ...
    case STATE_TOKEN_PENDING:
        ...
        if (client->token_get(&client->config, client->binding) != OPRT_OK) {
            printf("Get token fail, retry..");
            break;
        }

        printf("token: %s", client->binding->token);
        printf("region: %s", client->binding->region);
        printf("regist_key: %s", client->binding->regist_key);
...
int tuya_iot_activated_data_remove(tuya_iot_client_t* client)
{
    ...
    printf("Activated data remove successed");
...
int tuya_iot_version_update_sync(tuya_iot_client_t* client)
{
    ...
    if (OPRT_OK != rt) {
        printf("version save info not found:%d", rt);
    }

    /* Compare the version info changed? */
    if (memcmp(version_buffer, readbuf, version_len) == 0) {
        printf("The verison unchanged, dont need sync.");
        system_free(readbuf);
        system_free(version_buffer);
        return OPRT_OK;
    }
